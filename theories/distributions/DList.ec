...
lemma bigo_comp N f1 (f2:real -> real) : bigo N f1 \o f2 = foldr (fun n f2 => f1 n \o f2) f2 (range 0 N).
proof.
  rewrite /bigo.
  have -> //: forall f3, 
    (foldr (fun (n : int) (f2 : real -> real) => f1 n \o f2) f3
     (range 0 N) \o f2) =
    foldr (fun (n : int) (f2: real -> real) => f1 n \o f2) (f3 \o f2) (range 0 N).
  elim /natind: N f2.
  + by move=> n Hn f2 idfun;rewrite range_geq.
  by move=> n Hn Hrec f2 f3;rewrite rangeSr // -cats1 !foldr_cat /= Hrec.
qed.
...
lemma bigo_affine f N:
   (forall n, 0 <= n < N => affine (f n)) =>
   affine (bigo N f).
proof.
  elim /natind: N.
  + by move=> n Hn Hf;rewrite bigo0 // idfun_affine.
  move=> n Hn Hrec Hf;rewrite bigoS //.
  apply comp_affine=> /#.
qed.

...
and process1_phl (_ : ttenv) (t : phltactic located) (tc : tcenv1) =
  let (tx : tcenv1 -> tcenv) =
    ...
    | Pwhile_esp info           -> EcPhlWhile.process_esp_while info
    ...
    | Pconseq_esp info          -> EcPhlConseq.process_conseq_esp info
    | Pframe_esp(p,p',pnm)      -> EcPhlConseq.process_frame_esp p p' pnm

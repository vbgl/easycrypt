...
module PPEnv = struct
  ...
  let enter_pred_memdistr ppe f = 
    ...
    let ppe = push_distr ppe ~active:true (m,lmt) in
    (ppe, f)
...
let pp_distr (ppe : PPEnv.t) fmt x =
  let x = Format.sprintf "%s" (PPEnv.local_symb ppe x) in
  let x =
    if   x <> "" && x.[0] = '#'
    then String.sub x 1 (String.length x - 1)
...
(* -------------------------------------------------------------------- *)
let rec pp_bindings_blocks ppe ?fv bds =
  match bds with
  | [] ->
      (ppe, fun _ -> ())
  | [bd] ->
      let ppe, pp = pp_binding ppe ?fv bd in
      (ppe, fun fmt -> Format.fprintf fmt "%t" pp)
  | bd :: bds ->
      let ppe, pp1 = pp_binding ppe ?fv bd  in
      let ppe, pp2 = pp_bindings_blocks ppe ?fv bds in
      (ppe, fun fmt -> Format.fprintf fmt "%t@ %t" pp1 pp2)

let rec pp_bindings ppe ?fv bds =
...
and pp_ldform ppe fmt f = 
  let ppe, f = PPEnv.enter_pred_memdistr ppe f in
  pp_form ppe fmt f 
...
module PPGoal = struct
  ...
  let pp_goal1 ?(pphyps = true) ?(idx) (ppe : PPEnv.t) fmt (hyps, concl) =
    ...
    | FmuhoareF hf -> pp_muhoareF ppe fmt hf
    | FmuhoareS hs -> pp_muhoareS ppe fmt hs

...
(* -------------------------------------------------------------------- *)
let t_hS_or_bhS_or_eS ?th ?tbh ?te ?tesp tc =
  ...
  | FespS     _ when EcUtils.is_some tesp -> (oget tesp) tc
  ...
  | _ ->
    let kinds = List.flatten [
         ...
         if EcUtils.is_some tesp then [`Esp    `Stmt] else []]

    in tc_error_noXhl ~kinds !!tc

let t_hF_or_bhF_or_eF ?th ?tbh ?te ?tesp ?teg tc =
  match (FApi.tc1_goal tc).f_node with
  ...
  | FespF     _ when EcUtils.is_some tesp -> (oget tesp) tc
  ...
  | _ ->
    let kinds = List.flatten [
         ...
         if EcUtils.is_some tesp then [`Esp    `Pred] else [];

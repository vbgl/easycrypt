...
module LDecl = struct
  ...
  let ld_subst s ld =
    ...
    | LD_modty (p, r) ->
        let gty    = Fsubst.gty_subst s (GTmodty (p, r)) in
        let (p, r) = gty_as_mod gty in
        LD_modty (p, r)
    | LD_hyp f ->
  ...
  let has_inld s = function
      (* FIXME: we should head-normalize ty *)
    | LD_var (ty, _) when is_tlocalmem ty ->
        Msym.mem s (lmt_bindings (destr_tlocalmem ty))
    | _ -> false
